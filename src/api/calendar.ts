...
// 获取calendar-app的基础URL
const getCalendarBaseURL = () => {
  return window.__APP_CONFIG__?.CALENDAR_API_URL || '/calendar'
}
...
export const getEventList = async (startDate: string, endDate: string) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '获取事件列表失败')
      return []
    }
  } catch (error) {
...
export const getMonthEvents = async (year: number, month: number) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '获取月度事件失败')
      return []
    }
  } catch (error) {
    console.error('获取月度事件失败:', error)
    ElMessage.error('获取月度事件失败')
    return []
  }
}
...
export const getEventDetail = async (id: number) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '获取事件详情失败')
      return null
    }
  } catch (error) {
...
export const createEvent = async (event: CalendarEvent) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '创建失败')
      return false
    }
  } catch (error) {
...
export const updateEvent = async (event: CalendarEvent) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '更新失败')
      return false
    }
  } catch (error) {
...
export const deleteEvent = async (id: number) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '删除失败')
      return false
    }
  } catch (error) {
...
export const updateEventStatus = async (id: number, status: number) => {
  try {
    const ret = await $post(getCalendarBaseURL() + `/event/status/${id}?status=${status}`, {})
    if (ret.code === 1) {
      ElMessage.success('状态更新成功')
      return true
    } else {
      ElMessage.error(ret.codeMessage || '状态更新失败')
      return false
    }
  } catch (error) {
...
export const getReminderList = async () => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '获取提醒列表失败')
      return []
    }
  } catch (error) {
...
export const createReminder = async (reminder: Reminder) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '创建提醒失败')
      return false
    }
  } catch (error) {
...
export const cancelReminder = async (id: number) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '取消提醒失败')
      return false
    }
  } catch (error) {
...
export const getCyclePlanList = async () => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '获取周期计划失败')
      return []
    }
  } catch (error) {
    console.error('获取周期计划失败:', error)
    ElMessage.error('获取周期计划失败')
    return []
  }
}
...
export const createCyclePlan = async (plan: CyclePlan) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '创建周期计划失败')
      return false
    }
  } catch (error) {
    console.error('创建周期计划失败:', error)
    ElMessage.error('创建周期计划失败')
    return false
  }
}
...
export const pauseCyclePlan = async (id: number) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '暂停失败')
      return false
    }
  } catch (error) {
    console.error('暂停周期计划失败:', error)
    ElMessage.error('暂停周期计划失败')
    return false
  }
}
...
export const resumeCyclePlan = async (id: number) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '恢复失败')
      return false
    }
  } catch (error) {
    console.error('恢复周期计划失败:', error)
    ElMessage.error('恢复周期计划失败')
    return false
  }
}
...
export const getUpcomingPlans = async (hours: number = 24) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '获取即将触发计划失败')
      return []
    }
  } catch (error) {
    console.error('获取即将触发计划失败:', error)
    ElMessage.error('获取即将触发计划失败')
    return []
  }
}
