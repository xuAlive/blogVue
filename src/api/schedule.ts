...
// 获取schedule-app的基础URL
const getScheduleBaseURL = () => {
  return window.__APP_CONFIG__?.SCHEDULE_API_URL || '/schedule'
}
...
export const getShiftList = async () => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '获取班次列表失败')
      return []
    }
  } catch (error) {
...
export const getScheduleList = async (startDate: string, endDate: string) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '获取排班列表失败')
      return []
    }
  } catch (error) {
...
export const getMySchedules = async (startDate: string, endDate: string) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '获取排班失败')
      return []
    }
  } catch (error) {
...
export const batchCreateSchedules = async (schedules: Schedule[]) => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '批量创建失败')
      return false
    }
  } catch (error) {
...
export const getEmployeeList = async () => {
  try {
    ...
    if (ret.code === 1) {
      ...
      ElMessage.error(ret.codeMessage || '获取员工列表失败')
      return []
    }
  } catch (error) {
